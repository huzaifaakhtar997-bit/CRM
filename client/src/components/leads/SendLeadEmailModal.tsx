...
import { Lead, ConversationChannel, SenderType } from "../../types/api.types";
...
export const SendLeadEmailModal: React.FC<SendLeadEmailModalProps> = ({
  lead,
  isOpen,
  onClose,
  onSuccess,
  ...
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!lead.email) {
      setError("This lead does not have a valid email address.");
      return;
    }
    if (!subject.trim()) {
    ...
    try {
      // 1. Create conversation linked to this lead
      ...
      // 2. Create the first outbound message (triggers Resend delivery via backend)
        ...
        {sentSuccess ? (
          <div className="p-8 text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Your message was sent to <strong className="text-foreground">{lead.email}</strong> and a new conversation thread was created in your Unified Inbox.
            </p>
          ...
          <form onSubmit={handleSend} className="p-6 space-y-4">
            ...
            {!lead.email && (
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 text-amber-600 rounded-lg text-sm flex items-start gap-2">
                ...
                <span>
                  This lead does not have an email address. Edit the lead to add one before sending.
                </span>
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t">
              ...
              <Button
                type="submit"
                disabled={sending || !lead.email || !subject.trim() || !content.trim()}
                className="gap-2"
              >
                {sending ? (
                  "Sending..."
